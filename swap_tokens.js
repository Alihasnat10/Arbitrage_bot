...
async function swapTokens(token1, token2, amount, action, slippage = "50") {
    ...
    try {
        ...
        console.log(route)
        let amountIn = web3.utils.toWei(amount.toString())
        //let amountIn = ethers.utils.parseEther(amount.toString());
       
        if (action === 'buy') {amountIn = ethers.utils.parseEther(amount.toString());} //helper function to convert ETH to Wei
        ...
        if (action === 'buy') {
            ...
            console.log("Raw SELL")
        }
        else if(action === 'sell_tokens') {
            console.log("sell tokens")
            console.log(valueHex)
            rawTxn = await UNISWAP_ROUTER_CONTRACT.populateTransaction.swapTokensForExactTokens(valueHex, amountOutMinHex, path, to, deadline,{

                gasPrice: wallet.getGasPrice(),
                gasLimit: ethers.BigNumber.from(1000000).toHexString()
            })
            console.log("Raw SELL Tokens")
        }
        console.log(rawTxn)
...
swapTokens(DAI, WETH[DAI.chainId], 0.001, "buy")
